...
def get_gif_file_name() -> str:
    now = datetime.datetime.now()
    timestamp = now.strftime("%Y-%m-%dT%H%M%S")
    filename = f"image_{timestamp}.gif"
    return filename
...
def stop_recording(pipeline: Gst.Pipeline) -> None:
    valve = pipeline.get_by_name("valve")
    queue_filesink = pipeline.get_by_name("queue_filesink")
    gifenc = pipeline.get_by_name("gifenc")
    sink = pipeline.get_by_name("sink")
    ...
    for e in (queue_filesink, gifenc, sink):
        e.set_state(Gst.State.NULL)


def resume_recording(pipeline: Gst.Pipeline) -> None:
    valve = pipeline.get_by_name("valve")
    queue_filesink = pipeline.get_by_name("queue_filesink")
    gifenc = pipeline.get_by_name("gifenc")
    sink = pipeline.get_by_name("sink")
...
def rotate_recording(pipeline: Gst.Pipeline) -> None:
    stop_recording(pipeline=pipeline)
    resume_recording(pipeline=pipeline)


def configure_timers(pipeline: Gst.Pipeline) -> None:

    def on_interval():
        rotate_recording(pipeline=pipeline)
        return True
